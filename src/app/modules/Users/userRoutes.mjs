...
route.patch("/following/:id", async (req, res) => {
  ...
  const followingResult = await userSchema.updateOne(
    query1,
    followingUpdateDoc,
    options
  );
  const followerResult = await userSchema.updateOne(
    query2,
    followerUpdateDoc,
    options
  );
...
route.patch("/unfollow/:id", async (req, res) => {
  ...
  const Following = unFollowingResult?.Followings.filter(
    (e) => e != unFollowerId
  );
  const Follower = unFollowerResult?.Followers.filter(
    (e) => e != unFollowingId
  );
